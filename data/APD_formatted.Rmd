...
APD <- read_csv("APD_2023_02_13.csv")
words <- read_csv("ontology_links.csv")
reviewers <- read_csv("APD_reviewers.csv")
TOP <- read_csv("TOP_terms.csv")
units <- read_csv("units_links.csv")
references <- read_csv("APD_references.csv")
categorical_values <- read_csv("APD_categorical_values.csv") 

traits_selected <- "seed_length"
trait_i <- APD %>% filter(trait == traits_selected)
categorical_i <- categorical_values %>% filter(trait_name == traits_selected) %>% select(allowed_values, value_definitions)
```
...
**[trait description](http://purl.org/dc/terms/description)**:  

```{r, results='asis', echo=FALSE}
 if(!is.na(trait_i$description)){
   cat('- ')
   trait_i$description
  }  
```
  
- `r trait_i$description_encoded`  

**[comments](http://www.w3.org/2000/01/rdf-schema#comment)** :
```{r, results='asis', echo=FALSE}
if(!is.na(trait_i$comments)){
  trait_i$comments
}else{
  cat(' no comments')
}    
...
if(trait_i$type_x == "numeric"){
  cat('[**units:**](http://terminologies.gfbio.org/terms/ETS/expectedUnit)')
  if(is.na(match(trait_i$units_UCUM, units$code_to_match))) {
  trait_i$units
  } else {
  sprintf("[%s](%s)", trait_i$units, units$Entity[match(trait_i$units_UCUM, units$code_to_match)])  
  }
}  
```
...
```{r, results='asis', echo=FALSE}
if(trait_i$type_x == "numeric"){
  cat('[**allowable minimum value: **](http://terminologies.gfbio.org/terms/ETS/minAllowedValue)')
  trait_i$min
}  
```
...
```{r, results='asis', echo=FALSE}
if(trait_i$type_x == "numeric"){
  cat('[**allowable maximum value: **](http://terminologies.gfbio.org/terms/ETS/maxAllowedValue)')
  trait_i$max
}  
```
...
cat('[**constraints**](http://www.w3.org/2004/02/skos/core#scopeNote): ') 
...
cat('[**deprecated trait labels**](http://www.w3.org/2004/02/skos/core#changeNote): ') 
if(is.na(trait_i$deprecated_trait_name)) {
  cat('none')
} else {
trait_i$deprecated_trait_name    
}
...
```{r, results='asis', echo=FALSE, message=FALSE, warning=FALSE}
  ...
  if(nrow(refs) > 0) {
    cat('**[references](http://purl.org/dc/terms/references)**:') 
    refs %>%  kable(col.names = NULL)
  } else {  
    cat('**[references](http://purl.org/dc/terms/references)**: No references linked to this trait.\n')
  }
```
<br>
...
exact_match <- trait_i %>%
  ...
  filter(value != "NA NA") %>% 
  distinct() %>%
  rename('[**exact matches **](http://www.w3.org/2004/02/skos/core#exactMatch)' = value)
...
close_match <- trait_i %>%
  ...
  filter(value != "[NA]NA") %>%
  distinct() %>%
  rename('[**close matches **](http://www.w3.org/2004/02/skos/core#closeMatch)' = value)
...
related_match <- trait_i %>%
  ...
  filter(value != "[NA]NA") %>%
  distinct() %>%
  rename('[**related matches **](http://www.w3.org/2004/02/skos/core#relatedMatch)' = value)

cat('**matches to other databases**: ')

if(!nrow(exact_match>0)&!nrow(close_match>0)&!nrow(related_match>0)){
  cat('No matches to other trait databases documented. \n')
}

if(nrow(exact_match)>0) {
 exact_match %>% my_kable_styling()
}

if(nrow(close_match)>0) {
 close_match %>% my_kable_styling()
}

if(nrow(related_match)>0) {
 related_match %>% my_kable_styling()
}
...
[**reviewers**](http://purl.org/datacite/v4.4/IsReviewedBy):
```{r, results='asis', echo=FALSE, message=FALSE, warning=FALSE}
  if(nrow(reviews)>0) {
    reviews$joined
  } else {
    cat('Trait has been reviewed by the AusTraits team, but not an external reviewer.')
  }
```