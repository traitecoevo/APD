...
```{r}
triples_with_labels %>%
  write_csv("docs/triples_with_labels.csv")

triples_df %>%
  write_delim("docs/APD.nq", col_names=FALSE, escape="none", quote="none")

triples_df %>%
  select(-graph) %>%
  write_delim("docs/APD.nt", col_names = FALSE, escape = "none", quote = "none")

# prove this parses correctly
true_triples <- read_nquads("docs/APD.nq")

# serialize to any format
rdflib::rdf_serialize(true_triples, "docs/APD.ttl",
...
rdflib::rdf_serialize(true_triples, "docs/APD.json", format="jsonld")
```
...
```{r}
quarto::quarto_render("index.qmd", output_file = "all")
```
