---
title: "Exploring the APD and extracting trait definitions and metadata"
format:  
  pdf:
    toc: false
...
path <- "https://raw.githubusercontent.com/traitecoevo/APD/post_review_edits/docs/"

APD_traits <- read_csv(file.path(path, "APD_traits.csv"),
              show_col_types = FALSE)

APD_categorical_values <-  read_csv(file.path(path, "APD_categorical_values.csv"),
              show_col_types = FALSE)
```
...
stomatal_traits <- 
  APD_traits %>%
  dplyr::filter(stringr::str_detect(trait_groupings, 
                                    "leaf stomatal complex morphology trait"))
```
...
life_history %>%
  kableExtra::kable(format = "latex", booktabs=T, longtable = T) %>%
...
fluorescence %>% 
  ...
  column_spec(2, width = "3.9cm") %>%
  column_spec(3, width = "7cm")
```

Alternatively, if you know the doi for a reference, you can search for traits that reference it.
