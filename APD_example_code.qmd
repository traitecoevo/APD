...
The first 10 traits on this list:
...
Generate list of `trait_grouping` terms used:
...
Display all `leaf stomatal complex morphology` traits:
...
The term `characteristic_measured` documents "what" is being measured - a mass, length, etc. Multiple terms may be used, to indicate a trait captures volume and is a ratio:

```{r, output=FALSE}

volume_ratio_traits <- APD_traits %>%
...
To identify all traits that are also in the TRY trait database, with the first 20 displayed:
...
Slightly different code is required to extract lists of formally published trait definitions that have been matched to. The first 20 matches to the Woody Plants Ontology are:
...
deprecated_trait_names <- APD_traits %>%
...
deprecated_fire_traits <- APD_traits %>%
  select(trait, label, deprecated_trait_name) %>%
