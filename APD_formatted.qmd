...
params:
  traits_selected: "leaf_epidermis_Ca_per_fresh_mass"
format:
   pdf:
    toc: false
    papersize: A4
...
if(trait_i$type_x == "numeric"){
  ...
  trait_i <-  trait_i %>%
    mutate(units_UCUM_stripped = stringr::str_remove_all(units_UCUM, "\\{H2O\\}|\\{CO2\\}|\\{count\\}|\\{N\\}|\\{P\\}|\\{biomass\\}|\\{leaf\\}"))
  
  ii <- match(trait_i$units_UCUM_stripped, units$code_to_match)
  if(is.na(ii)) {
    out <- trait_i$units
  } else {
...
trait_category <- trait_i %>%
  ...
  distinct()
```
**trait group**: `r trait_category$value[1]`     
...
**[keywords](http://semanticscience.org/resource/SIO_000147)**: `r keywords$joined`   
...
exact_match <- trait_i %>%
  select(trait, exact_TOP, exact_TRY, exact_LEDA, exact_GIFT, exact_other1) %>% 
  mutate(
    exact_TOP = ifelse(!is.na(exact_TOP),paste0("[TOP](https://top-thesaurus.org/index): ", TOP$label[match(trait_i$exact_TOP, TOP$traitID)]," [", trait_i$exact_TOP,"]"),NA),
    exact_other1 = ifelse(!is.na(exact_other1), paste0("[",words$identifier[match(trait_i$exact_other1, words$base_identifier_to_match)],"](", words$url[match(trait_i$exact_other1, words$base_identifier_to_match)], "): ", words$term[match(trait_i$exact_other1, words$base_identifier_to_match)]), NA),
...
close_match <- trait_i %>%
  select(trait, close_TOP, close_TRY, close_LEDA, close_GIFT, close_other1, close_other2) %>%
  mutate(
    close_TOP = ifelse(!is.na(close_TOP),paste0("[TOP](https://top-thesaurus.org/index): ", TOP$label[match(trait_i$close_TOP, TOP$traitID)]," [", trait_i$close_TOP,"]"),NA),
    close_other1 = ifelse(!is.na(close_other1), paste0("[",words$identifier[match(trait_i$close_other1, words$base_identifier_to_match)],"](", words$url[match(trait_i$close_other1, words$base_identifier_to_match)], "): ", words$term[match(trait_i$close_other1, words$base_identifier_to_match)]), NA),
    close_other2 = ifelse(!is.na(close_other2), paste0("[",words$identifier[match(trait_i$close_other2, words$base_identifier_to_match)],"](", words$url[match(trait_i$close_other2, words$base_identifier_to_match)], "): ", words$term[match(trait_i$close_other2, words$base_identifier_to_match)]), NA),
...
related_match <- trait_i %>%
  ...
  mutate(
    related_TOP = ifelse(!is.na(related_TOP),paste0("[TOP](https://top-thesaurus.org/index): ", TOP$label[match(trait_i$related_TOP, TOP$traitID)]," [", trait_i$related_TOP,"]"),NA),
    related_TOP2 = ifelse(!is.na(related_TOP2),paste0("[TOP](https://top-thesaurus.org/index): ", TOP$label[match(trait_i$related_TOP2, TOP$traitID)]," [", trait_i$related_TOP2,"]"),NA),
    related_other = ifelse(!is.na(related_other), paste0("[",words$identifier[match(trait_i$related_other, words$base_identifier_to_match)],"](", words$url[match(trait_i$related_other, words$base_identifier_to_match)], "): ", words$term[match(trait_i$related_other, words$base_identifier_to_match)]), NA),
