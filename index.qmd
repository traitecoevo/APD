...
# One document holding the whole dictionary. Wenk et al. 2024 (p.12) commits to
# "a human-readable compiled version of the information contained in
# APD_triples.csv", so this page is part of the published contract, and every
# trait URI published since 2024 resolves to a fragment of it -- see
# COMMITMENTS.md C1 and C5.
#
# `embed-resources` stays: the copy archived in release/<version>/ is served at
# w3id.org/APD/release/<version>/index.html and deposited at Zenodo, and it has
# to open standalone without site_libs/ beside it.
format:
  html:
    smooth-scroll: true
    toc: true
...
# Directory form, not `.../index.html`: both resolve, but /APD/ is the canonical
# URL of this document and the one w3id redirects to, so publishing the other form
# here would cost readers an extra hop to reach the page's own address.
link_version <- function(v) sprintf("<https://w3id.org/APD/release/%s/>", v)
```
## Release `r version`

<dl>
<dt> **Latest version**:</dt> <dd><https://w3id.org/APD/> </dd>
...
# The generated artefacts live in export/ rather than at the repo root; they are
# still published at the site root, so the download links below are unchanged.
triples_with_labels <-
  read_csv(file.path(APD_EXPORT_DIR, "APD_triples.csv"), show_col_types = FALSE)
