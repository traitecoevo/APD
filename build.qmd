...
rdflib::rdf_serialize(true_triples, "APD.ttl",
                      namespace = c(APD = "https://w3id.org/APD/traits/",
                                    ...
                                    CO_357 = "https://cropontology.org/rdf/CO_357:",
...
exact1 <- 
  APD_traits_input %>%
  dplyr::select(trait, c(exact_other)) %>%
  dplyr::filter(!is.na(exact_other)) %>%
  dplyr::mutate(
    exact_other = stringr::str_replace(exact_other, CO:357:, ""),
    exact_other = stringr::str_split(exact_other, "; ")
    ) %>%
  tidyr::unnest_longer(exact_other) %>%
  dplyr::mutate(
    exact_other = paste0(
      published_classes$label[match(exact_other, published_classes$identifier)], " [",
      published_classes$Entity[match(exact_other, published_classes$identifier)], "]"
    )
  ) %>%
  dplyr::rename(value = exact_other)
...
close1 <- 
  ...
  dplyr::mutate(
    close_other1 = stringr::str_replace(close_other1, CO:357:, ""),
    close_other1 = paste0(
...
close2 <- 
  ...
  dplyr::mutate(
    close_other2 = stringr::str_replace(close_other1, CO:357:, ""),
    close_other2 = paste0(
...
related1 <- 
  ...
  dplyr::mutate(
    related_other = stringr::str_replace(related_other, CO:357:, ""),
